  ...
  private static Mono<FoodInfoEntity> validateAndUpdateAdditionInfo(
    ...
    return Mono.just(entity)
        ...
        .switchIfEmpty(
            Mono.error(
                new BadRequestException(
                    ExceptionMessages.INVALID_TEXT_LENGTH_MESSAGE.getMessage()
                        + "for largeInfo.")));
  }
  ...
  private static Mono<FoodInfoEntity> validateAndUpdatePicture(
    ...
    return Mono.just(entity)
        .filter(
            u ->
                !dto.picture().isBlank()
                    && (dto.picture().matches(imageUrlRegex)
                        || dto.picture().matches(imageUrlRegex2)))
        .flatMap(
            u -> {
              u.setPicture(dto.picture());
              return Mono.just(u);
            })
        .switchIfEmpty(Mono.error(new BadRequestException("Invalid picture")));
  }
}
